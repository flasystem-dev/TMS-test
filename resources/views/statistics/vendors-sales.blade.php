    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card card-body">
                <div class="page_top">
                    <div class="vendors_info">
                        @foreach($vendors_count as $row)
                        <div class="vendors_brand">
                            <div class="vendors_brand_text">
                                <p>
                                    <input type="radio" class="btn-check" name="brand" id="vendor_{{$row['brand']}}" value="{{$row['brand']}}" autocomplete="off" {{$brand===$row['brand'] ? "checked" : ""}}>
                                    <label class="btn vendor_type_text vendor_{{$row['brand']}}" for="vendor_{{$row['brand']}}">{{CommonCodeName($row['brand'])}}</label>
                                </p>
                            </div>
                            <div class="vendors_count">
                                <p class="new_vendors_text">사업자(신규)</p>
                                <p class="vendors_count">{{number_format($row['count'])}}명({{number_format($row['new'])}}명)</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
                        ...
                        <tbody>
                            @foreach($vendors as $vendor)
                                <tr>
                                    ...
                                    <td class="text-center"><span class="fw-bold possible_click" onclick="search_name('{{$vendor->recommend_name}}')">{{$vendor->recommend_name}}</span>({{number_format($vendor->recommend_person_count)}}명)</td>
...
@section('script')
    ...
    <script src="{{asset('assets/js/statistics/vendor-sales.js')}}"></script>
@endsection