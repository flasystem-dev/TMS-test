                        ...
                        @foreach($vendors as $vendor)
                            <tr class="text-center align-middle px-0">
                                ...
                                <td data-order="{{$vendor -> order_amount()}}">
                                    {{number_format($vendor -> order_amount())}}
                                    <br>
                                    {{number_format($vendor -> vendor_amount())}}
                                </td>
                                <!-- 옵션금액 -->
                                <td>
                                    {{number_format($vendor -> option_amount())}}
                                </td>
                                <!-- 수수료 -->
                                <td>
                                    {{$vendor -> service_percent()}} %
                                </td>
                                <!-- 차액수익 -->
                                <td>
                                    {{number_format($vendor->difference_amount() )}}
                                </td>
                                <!-- 수익총액(기타1 제외) -->
                                <td style="background-color: #fff0f0">
                                    {{number_format($vendor->revenue_amount())}}
                                </td>
                                <!-- 원천징수합계 -->
                                <td>
                                    {{number_format($vendor -> tax_amount())}}
                                </td>
                                <!-- 서비스 이용료 -->
                                <td>
                                    {{number_format($vendor -> service_fee(request()->year ?? date('Y'), request()->month ?? date('m')))}}
                                </td>
                                <!-- 카드 수수료 -->
                                <td class="cursor_p" onclick="reCalculate_cardAmount({{$year}}, {{$month}}, '{{$vendor->idx}}')">
                                    {{number_format($vendor -> card_fee())}}
                                    <br>
                                    ({{number_format($vendor -> card_amount())}})
                                </td>
                                <!-- 공제 총액 -->
                                <td style="background-color: #ebebff">
                                    {{number_format($vendor -> deduction_amount(request()->year ?? date('Y'), request()->month ?? date('m')))}}
                                </td>
                                <!-- 기타1 -->
                                <td>
                                    {{number_format($vendor -> etc1_amount())}}
                                </td>
                                <!-- 기타2 -->
                                <td>
                                    {{number_format($vendor -> etc2_amount())}}
                                </td>
                                <!-- 기타3 -->
                                <td>
                                    {{number_format($vendor -> etc3_amount())}}
                                </td>
                                <!-- 입금예정일 -->
                                <td>
                                    {{$vendor->deposit_date}}
                                ...
                                <td style="background-color: #ebffeb">
                                    {{number_format($vendor->settlement_amount(request()->year ?? date('Y'), request()->month ?? date('m')))}}
                                </td>
                                <td>
