...
class IntranetService
{
    ...
    public static function makeBaljuData($order,OrderProduct $orderProduct,  $input)
    {
        ...
        $data = [
            ...
            'rw_jname'          => $order->orderer_name,          // 주문자
            'rw_jtel'           => $order->orderer_tel,           // 주문전화
            'rw_jhandtel'       => $order->orderer_phone,         // 주문핸드폰
            'rw_aname'          => $order->delivery->receiver_name,         // 받는분
            'rw_atel'           => $order->delivery->receiver_tel,          // 받는분전화
            'rw_ahandtel'       => $order->delivery->receiver_phone,        // 받는분핸드폰
            'rw_arrive_place1'  => $order->delivery->delivery_address,      // 배송지1
            'rw_kyungjo'        => $order->delivery->delivery_ribbon_right, // 경조사어
            'rw_sendpeople'     => $order->delivery->delivery_ribbon_left,  // 보내는분
            'rw_card'           => $order->delivery->delivery_card,         // 카드메세지
//            'add2'              => '',                     // 메모
