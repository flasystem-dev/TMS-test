...
class OrderDataBatchExport implements FromQuery, WithHeadings, WithMapping, WithChunkReading, ShouldAutoSize, WithColumnFormatting, WithDefaultStyles, WithStyles
{
    ...
    public function __construct($order_idx, $downloadId)
    {
        ...
        $this->totalChunks = ceil(count($order_idx) / $this->chunkSize());
    }
    ...
    {
        try {
            if (($this->rowNumber - 2) % $this->chunkSize() === 0) {
                $progress = $this->chunkNumber/$this->totalChunks * 100;
                $this->chunkNumber++;

                OrderExcelDownload::where('id', $this->downloadId)->update(['progress'=>$progress]);
            }

            if (is_null($order->item)) {
                \Log::info("아이템 실패");
                \Log::info("주문 번호 : " . $order->order_idx);
            }
            ...
            \Log::error("[엑셀 일괄 다운로드 오류 발생]");
            \Log::error("주문인덱스 : ". $order->order_idx);
