...
class TestPlayAuto2APIController extends Controller
{
    ...
    {
        $site_code_list = DB::table('playauto2_api') -> select('site_code') -> groupBy('site_code') -> get();

        foreach ($site_code_list as $site_code) {
            $code = $site_code -> site_code;
            $site_id_list = DB::table('playauto2_api') -> select('site_id') -> where('site_code', '=', $code) -> get();
        ...
        try {
            ...
            if(!empty($data_arr)) {
                ...
                foreach ($data_arr as $result_data) {
                    ...
                    // 주문 DB 에 저장
                    self::set_order_ToDB($result_data);
                }
            }
    ...
    public static function setDeliveryState(){
        ...
        // 운송장 전송 (배송중 상태 변경)
        $result2 = self::send_delivery_toPT($order);
    ...
    {
        $site_code_list = DB::table('playauto2_api') -> select('site_code') -> groupBy('mall_code') -> get();
...
//        dd($obj);
    ...
    // 새로 받은 주문 DB 저장
    public static function set_order_ToDB($result_data) {
        ...
        for($i=0; $i<$result_data['sale_cnt']; $i++) {
            ...
            $od_id = (int)(microtime(true)*10) . Str::padLeft($i, 2, '0');
            ...
            $order -> payment_date              = $result_data['pay_time'];
            $order -> total_amount              = $result_data['sales']/$result_data['sale_cnt'];
            $order -> pay_amount                = $result_data['sales']/$result_data['sale_cnt'];
            $order -> supply_amount             = $result_data['shop_supply_price']/$result_data['sale_cnt'];
            $order -> admin_regist              = 'PLTO';
            $order -> playauto_date             = $result_data['wdate'];
            $order -> create_ts                 = NOW();
            $order -> options_string            = $result_data['shop_opt_name'];
            $order -> options_parse_yn          = $option_parse_yn;
            $order -> options_string_display    = $option_display;
            $order -> open_market_goods_url     = $result_data['shop_sale_no'];
            $order -> db_num                    = $result_data['bundle_no'];
            $order -> new_order_yn              = 'Y';

            $delivery -> order_idx              = $order_idx;
            $delivery -> order_delivery_number  = 1;
            $delivery -> receiver_name          = $receiverData['name'] != '' ? $result_data['to_name'] ."/".$receiverData['name'] : $result_data['to_name'];
            $delivery -> receiver_tel           = $result_data['to_htel'];
            $delivery -> receiver_phone         = $receiverData['phone'] != '' ? $receiverData['phone'] : $result_data['to_htel'];
            ...
            $delivery -> delivery_address       = $result_data['to_addr1']." ".$result_data['to_addr2'];
            ...
            $delivery -> delivery_state_code    = "DLUD";
...
//        dd($result['results']);
    ...
    {
        $orders = DB::table('order_data') -> where('order_number', "=", $result_data['shop_ord_no']) -> get();
        $cnt = count($orders);
...
//            $deliveryTime['deliveryTime']['date'] = $todayFormat;
    ...
    public static function useGoogleAPI()
    {
        ...
        $languageClient = new LanguageClient([
            'keyFilePath' => '/var/www/flasystem_net/google_keyFile.json',
...
//                    $day_arr = self::removeAM_PM($day);
    ...
    // 출고 지시
    public static function set_delivery() {
    ...
    public static function set_memo() {
        $uniq = "3942307592748026999";

        $response = Http::withHeaders([
            'x-api-key' => self::$api_key,
            'Authorization' => 'Token '.self::get_token()
        ]) -> post('https://openapi.playauto.io/api/memo', [
            'memo_type' => 'ord',
            'content' => '전송',
            'uniq' => $uniq
        ]);
    ...
    public static function updateCancelData($orderData) {
        ...
        if(empty($orderData['shop_ord_no_real'])){
